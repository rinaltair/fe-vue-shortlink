<template>
  <div class="comment-module">
    <form @submit.prevent="addComment">
      <div>
        <input v-model="newComment.author" placeholder="Your name" required />
        <textarea v-model="newComment.content" placeholder="Say something..." required></textarea>
...
<script setup lang="ts">
  ...
  const addComment = () => {
    if (newComment.value.author && newComment.value.content) {
      ...
      alert('Please complete the comment information')
    }
  }
  ...
  const addReply = (commentId: number, replyAuthor: string, replyContent: string) => {
    ...
    if (comment && replyAuthor && replyContent) {
      ...
      alert('Please complete the reply information')
    }
  }

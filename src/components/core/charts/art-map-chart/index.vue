...
<template>
  <div class="art-map-chart" :style="{ height: 'calc(100vh - 120px)' }">
    <div v-if="isEmpty" class="chart-empty-state">
      <ElEmpty description="No map data" />
    </div>
...
<script setup lang="ts">
  ...
  // Prepare map data based on geoJson
  ...
  // Get theme-related style configuration
  ...
  const createChartOption = (mapData: any[]) => {
    ...
    return {
      animation: false, // Disable animation to reduce frame drops on hover
      tooltip: {
        show: true,
      ...
      series: [
        ...
        // Scatter marker configuration (e.g., city markers)
        ...(props.showScatter
          ? [
              {
                name: 'Cities',
  ...
  // Watch theme changes and re-initialize the map
